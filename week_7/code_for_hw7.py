...
class Linear(Module):
    ...
    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        self.dLdW  = self.A@dLdZ.T  # dLdW is (m x n)
        self.dLdW0 = np.sum(dLdZ, axis=1).reshape(-1, 1)  # dLdW0 is (n x 1)
...
class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = np.maximum(0, Z)  # Your code: (m, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        return dLdA*(self.A != 0)  # Your code: return dLdZ (m, b)
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred
        self.Y = Y
        # return np.sum(-Y*np.log(Ypred) + (1-Y)*np.log(1-Ypred)).item()  # Your code: return loss (scalar)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            A = X[:, i:i+1]
            labels = Y[:, i:i+1]
            Ypred = self.forward(A)
            loss_total += self.loss.forward(Ypred, labels)
            err = self.loss.backward()
            self.backward(err)
            self.sgd_step(lrate)
            self.print_accuracy(it, A, labels, loss_total, every=250)
