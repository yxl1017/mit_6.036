...
def perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        ...
        for i in range(labels.shape[1]):
            if (labels[0][i]*y(cv(data[:, i]), th, th0)).item() <= 0:
                th = th + labels[0][i]*cv(data[:, i])
                th0 = th0 + labels[0][i]
                changed = True
                if hook:
                    hook((th, th0))                
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    T = params.get('T', 100)
    # Your implementation here
    th = ths = np.zeros((data.shape[0], 1))
    th0 = th0s = np.array([[0]])
    for t in range(T):
        for i in range(labels.shape[1]):
            if (labels[0][i]*y(cv(data[:, i]), th, th0)).item() <= 0:
            ...
            ths = ths + th
            th0s = th0s +th0
    return ths/(T*labels.shape[1]), th0s/(T*labels.shape[1])
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i in range(k):
        data_test = D[i]
        labels_test = L[i]
        data_train = np.concatenate((D[:i] + D[i+1:]), axis=1)
        labels_train = np.concatenate((L[:i] + L[i+1:]), axis=1)
...
# for it in range(5, 100, 5):
#      print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.5), 20, 20, it))
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        data_train, labels_train = data_gen(n_train)
        #data_test, labels_test = data_gen(n_test)
        s = eval_classifier(learner, data_train, labels_train, data_train, labels_train)
        S += s
    return S/it

for i in (0.1, 0.25):
     print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=i), 20, 20, 100))
     print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=i), 20, 20, 100))
