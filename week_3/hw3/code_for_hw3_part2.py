...
def eval_classifier(learner, T, data_train, labels_train, data_test, labels_test):
    th, th0 = learner(data_train, labels_train, params = {'T': T})
    return score(data_test, labels_test, th, th0)/data_test.shape[1]

def xval_learning_alg(learner, T, data, labels, k):
    _, n = data.shape
    idx = list(range(n))
    np.random.seed(0)
    ...
    for i in range(k):
        ...
        score_sum += eval_classifier(learner, T, data_train, labels_train,
...
def get_classification_accuracy(data, labels):
    ...
    return xval_learning_alg(perceptron, 50, data, labels, 10)
